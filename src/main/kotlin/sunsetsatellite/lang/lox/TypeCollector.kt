...
class TypeCollector(val lox: Lox) : Stmt.Visitor<Unit> {
    ...
    override fun visitFunctionStmt(stmt: Stmt.Function) {
        ...
        addInfo(parent, stmt, Type.ofFunction(stmt.name.lexeme,lox), stmt.returnType, listOf())

        stmt.params.forEach { addInfo(stmt, Stmt.Var(it.token,it.type,null,FieldModifier.NORMAL), it.type, Type.UNKNOWN, listOf()) }
    ...
    override fun visitClassStmt(stmt: Stmt.Class) {
        ...
        val type = Type.ofClass(stmt.name.lexeme,lox)
        val returnType = Type.ofObject(stmt.name.lexeme,lox)
    ...
    override fun visitImportStmt(stmt: Stmt.Import) {
        lox.imports[stmt.what.literal]?.let { collect(it, stmt.what.literal.toString()) }
    }
