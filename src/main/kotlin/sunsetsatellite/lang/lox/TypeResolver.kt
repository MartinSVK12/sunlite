...
class TypeResolver(val collector: TypeCollector, val lox: Lox) : Stmt.Visitor<Type>, Expr.Visitor<Type> {
    ...
    override fun visitLambdaExpr(expr: Expr.Lambda): Type {
        return Type.Reference(PrimitiveType.FUNCTION, expr.function.name.lexeme, listOf(), lox)
    }
                ...
                if(info != null && info.returnType is Type.Parameter){
                    if(objType is Type.Reference){
                        ...
                        if(typeParamIndex > -1 && objType.typeParameters.size > typeParamIndex){
                            ...
                            lox.error(expr.name,"Unspecified type parameter ${info.returnType}.")
                        }
                    }
                }
            }
