...
import sunsetsatellite.lang.lox.TokenType.TokenGroup.*
...
enum class TokenType(val group: TokenGroup) {
	...
	LESS_PIPE(MULTI_CHAR), PLUS_EQUAL(MULTI_CHAR), MINUS_EQUAL(MULTI_CHAR), /*INCREMENT, DECREMENT,*/

	// Literals.
	IDENTIFIER(LITERALS), STRING(LITERALS), NUMBER(LITERALS),

	// Keywords.
	AND(KEYWORDS), CLASS(KEYWORDS), ELSE(KEYWORDS), FALSE(KEYWORDS), FUN(KEYWORDS), FOR(KEYWORDS), IF(KEYWORDS), NIL(KEYWORDS), OR(KEYWORDS),
	/*PRINT,*/ RETURN(KEYWORDS), SUPER(KEYWORDS), THIS(KEYWORDS), TRUE(KEYWORDS), VAR(KEYWORDS), WHILE(KEYWORDS),
	BREAK(KEYWORDS), CONTINUE(KEYWORDS), STATIC(KEYWORDS), INTERFACE(KEYWORDS), IS(KEYWORDS), IS_NOT(KEYWORDS),
	IMPORT(KEYWORDS), NATIVE(KEYWORDS), DYNAMIC(KEYWORDS), AS(KEYWORDS),

	// Type keywords
	TYPE_STRING(TYPES), TYPE_NUMBER(TYPES), TYPE_BOOLEAN(TYPES), TYPE_FUNCTION(TYPES), TYPE_ANY(TYPES), TYPE_ARRAY(TYPES),
