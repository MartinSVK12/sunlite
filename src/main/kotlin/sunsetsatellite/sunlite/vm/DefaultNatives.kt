    ...
    fun registerIO(vm: NativesContainer) {
        ...
        vm.defineNative(object : SLNativeFunction("File#readBytesN", Type.ofArray(Type.BYTE), 1) {
            override fun call(
                ...
                file.readBytes().let { return SLArrayObj(SLArray(it.size, vm.sunlite).overwrite(it.map { SLByte(it) }.toTypedArray())) }
            }
        })
    ...
    fun registerCore(vm: NativesContainer) {
        ...
        vm.defineNative(object : SLNativeFunction("launchArgs", Type.ofArray(Type.STRING), 0) {
            override fun call(
                ...
                return SLArrayObj(SLArray(vm.launchArgs.size, vm.sunlite).overwrite(vm.launchArgs.map { SLString(it) }
                    .toTypedArray()))
            }
        })
