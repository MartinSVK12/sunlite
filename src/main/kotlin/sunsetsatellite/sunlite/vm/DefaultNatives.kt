...
object DefaultNatives : Natives {
    ...
    fun registerIO(natives: NativesContainer) {
        natives.defineNative(object : SLNativeFunction("print", Type.NIL, 1) {
            override fun call(vm: VM, args: Array<AnySLValue>, typeArgs: Array<SLType>, receiver: SLClassInstanceObj?): AnySLValue {
                val value = args[0]
        ...
        natives.defineNative(object : SLNativeFunction("File#readBytes", Type.ofArray(Type.BYTE), 0) {
            override fun call(
                ...
                val file = ((receiver!!).value.fields["<foreign>fileHandle"]?.value as SLForeignObject).value as File
    ...
    fun registerReflect(natives: NativesContainer) {
        natives.defineNative(object : SLNativeFunction("Reflect#getMethodNames", Type.ofArray(Type.STRING), 1) {
            override fun call(vm: VM, args: Array<AnySLValue>, typeArgs: Array<SLType>, receiver: SLClassInstanceObj?): AnySLValue {
                ...
                if(clazz is SLClassObj){
                    array = clazz.value.methods.keys.map { SLString(it) }.toTypedArray()
                } else if(clazz is SLClassInstanceObj){
                    array = clazz.value.clazz.methods.map { SLString(it.key) }.toTypedArray()
                } else {
        ...
        natives.defineNative(object : SLNativeFunction("Field#loadField", Type.NIL, 0) {
            override fun call(vm: VM, args: Array<AnySLValue>, typeArgs: Array<SLType>, receiver: SLClassInstanceObj?): AnySLValue {
                ...
                val field: SLField
                if(c is SLClass){
	                if (c.staticFields[name] == null) {
                        vm.throwException("No static field '$name' in class '${c.name}'.")
                        return SLNil
                    }
	                field = c.staticFields[name]!!
                } else if(c is SLClassInstance){
                    if (c.fields[name] == null) {
	                    if (c.clazz.staticFields[name] == null) {
                            vm.throwException("No static or instance field '$name' in class '${c.clazz.name}'.")
		                    return SLNil
	                    } else {
                            field = c.clazz.staticFields[name]!!
                        }
                    } else {
                        field = c.fields[name]!!
                    }
