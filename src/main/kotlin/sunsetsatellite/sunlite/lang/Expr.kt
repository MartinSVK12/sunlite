...
abstract class Expr : Element {
    ...
    data class Literal(val value: Any?, val lineNumber: Int, val currentFile: String?, val type: Type = Type.UNKNOWN) :
        Expr() {
        override fun <R> accept(visitor: Visitor<R>): R? {
    ...
    data class Variable(val name: Token, val type: Type = Type.UNKNOWN, val constant: Boolean = false) : Expr(),
        NamedExpr {
        override fun <R> accept(visitor: Visitor<R>): R? {
    ...
    data class Call(val callee: Expr, val paren: Token, val arguments: List<Expr>, val typeArgs: List<Param>,
        ...
        override fun getExprType(): Type {
            ...
            if (type is Type.Reference) {
                ...
                /*if(type.type == PrimitiveType.FUNCTION){
                    if (!typeArgs.isEmpty()) {
                        return Type.reify(type, typeArgs)
                    }
                }*/
            }
            return Type.UNKNOWN
