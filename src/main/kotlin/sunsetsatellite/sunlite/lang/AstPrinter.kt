...
object AstPrinter : Expr.Visitor<String>, Stmt.Visitor<String> {
    ...
    override fun visitCallExpr(expr: Expr.Call): String {
        return "(call${if (expr.typeArgs.isEmpty()) "" else "<${expr.typeArgs.joinToString(", ")}>"} ${print(expr.callee)} -> ${expr.getExprType()} ${
            if (expr.arguments.isEmpty()) "(no args)" else parenthesizeList(
                "args",
                expr.arguments
            )
        })"
    ...
    override fun visitLambdaExpr(expr: Expr.Lambda): String {
        return parenthesize(
            "lambda ( ${expr.function.params.toString().replace("[", "").replace("]", "")})",
            expr.function.body
        )
    }

    override fun visitGetExpr(expr: Expr.Get): String {
        return "(get${if (expr.typeParams.isEmpty()) "" else "<${expr.typeParams.joinToString(", ")}>"} '${expr.name.lexeme}: ${expr.type}' ${
            print(
                expr.obj
            )
        })"
    ...
    private fun parenthesize(name: String, stmts: List<Stmt>): String {
        ...
        builder.append("(").append(name)
        if (name == "block" || name == "try" || name == "catch" || (FunctionType.entries.any {
                name.contains(
                    it.toString().lowercase()
                )
            } && !name.contains("abstract") && !name.contains("native")) || name.contains("class") || name.contains("interface") || name.contains(
                "lambda"
            )) {
            builder.append(" {\n")
            tabs++
    ...
    override fun visitVarStmt(stmt: Stmt.Var): String {
        return stmt.initializer?.let {
            parenthesize(
                "${stmt.modifier.name.lowercase()} var '${stmt.name.lexeme}': ${stmt.type} =",
                it
            )
    ...
    override fun visitClassStmt(stmt: Stmt.Class): String {
        return parenthesize(
            "${stmt.modifier.name.lowercase()} class${
                if (stmt.typeParameters.isEmpty()) "" else "<${
                    stmt.typeParameters.joinToString(
                        ", "
                    ) { it.token.lexeme }
                }>"
            } ${stmt.name.lexeme}${stmt.superclass?.let { return@let " < (superclass ${it.name.lexeme})" } ?: ""}${
                if (stmt.superinterfaces.isNotEmpty()) " << ${
                    parenthesizeList(
                        "superinterfaces",
                        stmt.superinterfaces
                    )
                }" else ""
            }",
    ...
    override fun visitTryCatchStmt(stmt: Stmt.TryCatch): String {
        return parenthesize("try", stmt.tryBody.statements) + parenthesize(
            "catch (${stmt.catchVariable})",
            stmt.catchBody.statements
        )
    }
