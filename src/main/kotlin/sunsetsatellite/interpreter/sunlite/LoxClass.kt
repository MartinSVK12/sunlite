...
class LoxClass(
    ...
    sunlite: Sunlite
): LoxClassInstance(null, sunlite, mapOf()), LoxCallable, LoxObject {
    ...
    override fun call(interpreter: Interpreter, arguments: List<Any?>?, typeArguments: List<Type>): Any {
        ...
        val instance = LoxClassInstance(this, sunlite, runtimeTypeMap)
    ...
    override fun set(name: Token, value: Any?) {
        if (staticFields.containsKey(name.lexeme)) {
            ...
            sunlite.typeChecker.checkType(field.type, Type.fromValue(value, sunlite),name,true)
            field.value = value
            return
        }
