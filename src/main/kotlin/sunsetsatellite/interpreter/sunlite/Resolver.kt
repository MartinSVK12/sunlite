...
class Resolver(private val interpreter: Interpreter, val sunlite: Sunlite): Expr.Visitor<Unit>, Stmt.Visitor<Unit> {
    ...
    override fun visitLambdaExpr(expr: Expr.Lambda) {
        declare(expr.function.name, Type.ofFunction(expr.function.name.lexeme, sunlite), expr.function.returnType, expr.function.params.map { it.type })
    ...
    override fun visitBreakStmt(stmt: Stmt.Break) {
        if(inLoop <= 0) sunlite.error(stmt.keyword, "Unexpected `break` outside of loop.")

        // nothing else to resolve
    }

    override fun visitContinueStmt(stmt: Stmt.Continue) {
        if(inLoop <= 0) sunlite.error(stmt.keyword, "Unexpected `continue` outside of loop.")

        // nothing else to resolve
    }

    override fun visitFunctionStmt(stmt: Stmt.Function) {
        declare(stmt.name, Type.ofFunction(stmt.name.lexeme, sunlite), stmt.returnType, stmt.params.map { it.type })
        define(stmt.name, Type.ofFunction(stmt.name.lexeme, sunlite), stmt.returnType, stmt.params.map { it.type })
    ...
    override fun visitReturnStmt(stmt: Stmt.Return) {
        if (currentFunction == FunctionType.NONE) {
            sunlite.error(stmt.keyword, "Unexpected `return` outside of function.")
            return
        }
        stmt.value?.let {
            if (currentFunction == FunctionType.INITIALIZER) {
                sunlite.error(stmt.keyword,
    ...
    override fun visitClassStmt(stmt: Stmt.Class) {
        ...
        declare(stmt.name, Type.ofClass(stmt.name.lexeme, sunlite))
        define(stmt.name, Type.ofClass(stmt.name.lexeme, sunlite))

        if (stmt.superclass != null && stmt.name.lexeme == stmt.superclass.name.lexeme) {
            sunlite.error(stmt.superclass.name,
                "A class can't inherit from itself.");
        }

        if(stmt.superinterfaces.any { return@any stmt.name.lexeme == it.name.lexeme }){
            sunlite.error(stmt.name,
        ...
        if (stmt.superclass != null) {
            beginScope();
            scopes.peek().computeIfAbsent("super") { ResolutionInfo(true, Type.ofObject(stmt.superclass.name.lexeme,sunlite), Type.UNKNOWN, listOf()) }
        }

        beginScope()
        scopes.peek().computeIfAbsent("this") { ResolutionInfo(true, Type.ofObject(stmt.name.lexeme,sunlite), Type.UNKNOWN, listOf()) }
    ...
    override fun visitImportStmt(stmt: Stmt.Import) {
        ...
        sunlite.imports[stmt.what.literal]?.let { resolve(it, stmt.what.literal.toString()) }
    }
    ...
    private fun define(name: Token, type: Type, returnType: Type = Type.UNKNOWN, paramTypes: List<Type>? = null) {
        if(Sunlite.debug) println("variable '${name.lexeme}' is of type '${type.getName()}'")
        if (scopes.isEmpty()) {
