...
class TypeChecker(val collector: TypeCollector, val sunlite: Sunlite) : Stmt.Visitor<Unit>, Expr.Visitor<Unit> {

    val resolver: TypeResolver = TypeResolver(collector, sunlite)
    ...
    override fun visitImportStmt(stmt: Stmt.Import) {
        sunlite.imports[stmt.what.literal]?.let { check(it, stmt.what.literal.toString()) }
    }
        ...
        if(expr.callee is Expr.NamedExpr){
            ...
            if(info.self.second is Stmt.Function){
                ...
                if(expr.arguments.size != params.size) {
                    sunlite.error(expr.paren, "Expected ${params.size} arguments but got ${expr.arguments.size}.")
                    return
                }
                ...
                params.forEachIndexed { index, it ->
                    ...
                    if(actualType is Type.Parameter && calleeType is Type.Reference){
                        ...
                        if(typeParamIndex > -1 && calleeType.typeParameters.size > typeParamIndex){
                            ...
                            sunlite.error(expr.paren,"Unspecified type parameter $actualType.")
                        }
                    }
    ...
    fun checkType(
        ...
        if(Sunlite.debug) println("checking if '${expectedName ?: token.lexeme}' of type '${expected.getName()}' matches type '${actual.getName()}'")
        val valid = Type.contains(actual, expected)

        if(expected is Type.Parameter || actual is Type.Parameter && !valid){
            sunlite.warn(token, "Unable to clarify concrete type of generic type, using '$actual' as '${expected}'.")
            return
        }
        ...
        if(actual == Type.UNKNOWN){
            ...
            sunlite.warn(token,"Unchecked cast, couldn't determine if ${if (expectedName != null) "'${expectedName}' of type is " else "type is "}'${expected.getName()}', got '${actual.getName()}'.")
            return
        }

        if(!valid){
            if(!runtime){
                sunlite.error(token, "Type mismatch, expected ${if(expectedName != null) "'${expectedName}' to be " else ""}'${expected.getName()}' but got '${actual.getName()}'.")
            } else {
        ...
        if(Sunlite.debug) println("type check succeeded")
    }
}