...
import sunsetsatellite.lang.sunlite.Expr.Logical
import sunsetsatellite.lang.sunlite.Stmt.Return
import sunsetsatellite.lang.sunlite.TokenType.*
import java.io.InputStreamReader
import java.nio.charset.StandardCharsets


class Interpreter(file: String?, val globals: Environment = Environment(null,0, "<global env>", file), val sunlite: Sunlite): Expr.Visitor<Any?>,
	Stmt.Visitor<Unit> {
		...
		if (distance != null) {
			when(expr.operator){
				...
				PLUS_EQUAL -> {
					...
					sunlite.typeChecker.checkType(
						Type.Union(listOf(Type.NUMBER, Type.STRING)),
						Type.fromValue(value, sunlite), expr.name)
					sunlite.typeChecker.checkType(
						Type.Union(listOf(Type.NUMBER, Type.STRING)),
						Type.fromValue(previous, sunlite), expr.name)
				...
				MINUS_EQUAL -> {
					val previous = environment.getAt(distance, expr.name.lexeme)
					sunlite.typeChecker.checkType(Type.NUMBER, Type.fromValue(value, sunlite), expr.name)
					sunlite.typeChecker.checkType(Type.NUMBER, Type.fromValue(previous, sunlite), expr.name)
					if(previous is Double && value is Double){
						environment.assignAt(distance, expr.name, previous - value)
					}
				}
	...
	fun interpret(statements: List<Stmt>, path: String?) {
		if(sunlite.hadRuntimeError) return
		currentFile = path
		try {
	...
	private fun execute(stmt: Stmt) {
		...
		if(sunlite.breakpoints[currentFile]?.contains(stmt.getLine()) == true){
			sunlite.breakpointListeners.forEach { it.breakpointHit(stmt.getLine(),currentFile,sunlite,environment) }
			breakpointHit = true
			while (true){
	...
	override fun visitFunctionStmt(stmt: Stmt.Function) {
		val function = LoxFunction(stmt, environment, sunlite)
	...
	override fun visitClassStmt(stmt: Stmt.Class) {
		...
		for (method in stmt.methods) {
			val function = LoxFunction(method, environment, sunlite).apply { if (method.name.lexeme == "init" && method.modifier != FunctionModifier.NATIVE) this.setModifier(
				FunctionModifier.INIT
			) }
			methods[method.name.lexeme] = function
		}

		val clazz = LoxClass(stmt.name.lexeme, methods, stmt.fieldDefaults.associate { it.name.lexeme to LoxField(it.type, it.modifier, evaluate(it.initializer)) }, superclass as LoxClass?, superinterfaces, stmt.modifier, stmt.typeParameters, sunlite)
	...
	override fun visitInterfaceStmt(stmt: Stmt.Interface) {
		...
		val methods: MutableMap<String, LoxFunction> = mutableMapOf()
		for (method in stmt.methods) {
			val function = LoxFunction(method, environment, sunlite).apply { this.setModifier(FunctionModifier.ABSTRACT) }
			methods[method.name.lexeme] = function
		}
	...
	override fun visitImportStmt(stmt: Stmt.Import) {
		sunlite.imports[stmt.what.literal]?.let { interpret(it, stmt.what.literal.toString()) }
	}
	...
	override fun visitLambdaExpr(expr: Expr.Lambda): Any {
		return LoxFunction(expr.function, environment, sunlite)
	}
	...
	override fun visitDynamicGetExpr(expr: Expr.DynamicGet): Any? {
		...
		if (obj is LoxArray) {
			sunlite.typeChecker.checkType(Type.NUMBER, Type.fromValue(what, sunlite),expr.token,true)
			return obj.get((what as Double).toInt(),expr.token)
		}

		throw LoxRuntimeError(
			expr.token,
			"Can only get either properties from classes or class instance, or elements from an array. Value is of type '${Type.fromValue(obj,sunlite)}'"
		)
	}
	...
	override fun visitDynamicSetExpr(expr: Expr.DynamicSet): Any? {
		...
		if(obj is LoxClassInstance) {
			...
			sunlite.typeChecker.checkType(Type.NUMBER, Type.fromValue(index, sunlite),expr.token,true)
			obj.set((index as Double).toInt(), value, expr.token)
		} else {
			throw LoxRuntimeError(
				expr.token,
				"Can only set either fields on classes or class instances, or elements into an array. Value is of type '${Type.fromValue(obj,sunlite)}'"
			)
		}
	...
	override fun visitCastExpr(expr: Expr.Cast): Any? {
		...
		val type = Type.fromValue(evaluated, sunlite)

		sunlite.typeChecker.checkType(type,expr.right,expr.operator,true)
